...
const m_nav = document.querySelector('#m_nav'); //붙여넣기 위치 ,id라 중복 대상이 되지 않으므로 부모 선택자를 붙여넣지 않았음

const cloneNav = full_nav.cloneNode(true); //복제실행
m_nav.appendChild(cloneNav) //붙여넣기
...
m_nav_btn.addEventListener('click',()=>{
    // 기존 css에서 어떤속성으로 숨겨져 있었는지에 따라 반대되는 값 작성하기
    // 기존 display:none -> 변경 display:block or flex
    // 기존 opacity:0 - > opacity:1
    // m_nav_bg.style.right='0%'; /* 단위가 붙으면 작은따옴표 붙인다 '0%' */
