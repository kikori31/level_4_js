...
topCTA.addEventListener('click',()=>{
    ...
    window.scrollTo({left:0, top:0, behavior:'smooth'}) //부드럽게 이동이 필요할 때
})
...
window.addEventListener('scroll',()=>{
    ...
    for(let s of section){
        ...
        const sTop = s.getBoundingClientRect().top; //거리 체크
    ...
    if(window.scrollY >= 400){ //스크롤이 400px 이상일때 CTA 보이고 400보다 작으면 CTA 숨김
